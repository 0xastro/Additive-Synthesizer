...
void DrawTask_Info(void) {
	...
	al_draw_textf(font, c2, 14, 80, 0, "Deadline Miss:    %d",deadline_miss(task_sepcs.id_audio));
	...
	al_draw_textf(font, c2, 14+180, 80, 0, "Deadline Miss:    %d",deadline_miss(task_sepcs.id_display));
	...
	al_draw_textf(font, c2, 14+192*3, 40, 0, "Sin         DLM:      %d",deadline_miss(task_sepcs.id_sinWave));
...
void Draw_Instructions(void) {
	...
	al_draw_textf(fonts,c1, 120, 480, ALLEGRO_ALIGN_LEFT,"SIN    TRIANGLE    SQUARE");  
